define redcurrant18::meta0 ($type='meta0',$action='create',$num='0',$options={prefixdir=>'/usr/local',bindir=>'/usr/local/bin',fhs_compliance=>'true'}) {
  ...
  redcurrant18::grid-init-service { "${options['ns']}-${type}-${num}":
    action => $action,
    command => "/usr/local/bin/meta0_server -v -p ${rundirname}/${type}-${num}.pid -s RC,${options['ns']},${options['stgdev']},${type}-${num} -O Endpoint=${options['ipaddr']}:${options['port']} ${options['ns']} ${options['vol']}", 
    enabled => 'true',
    start_at_boot => 'no',
    on_die => 'respawn',
